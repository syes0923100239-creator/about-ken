...
if st.session_state.calculated:
 ...
 if option == "Trade":
        ...
        with col_a1:
            ...
            st.info(get_action_text(diff_o, numbero))
            
        with col_a2:
            st.subheader("Double (30%)")
            st.write(f"Target: ${target_d:,.2f}")
            st.info(get_action_text(diff_d, numberd))

        with col_a3:
            st.subheader("Cash (30%)")
            st.write(f"Target: ${target_c:,.2f}")
            st.info(get_action_text(diff_c, numberc))
    ...
    # --- 方案 B：只投入新資金再平衡 (Investing money) ---
        ...
        with col_b1:
            ...
            st.info(get_action_text(diff_o_new, numbero))
            
        with col_b2:
            st.subheader("Double (30%)")
            st.write(f"New Target: ${new_target_d:,.2f}")
            st.info(get_action_text(diff_d_new, numberd))

        with col_b3:
            st.subheader("Cash (30%)")
            st.write(f"New Target: ${new_target_c:,.2f}")
            st.info(get_action_text(diff_c_new, numberc))
else:
 st.info("Please enter share prices and amounts to calculate proportions.")
